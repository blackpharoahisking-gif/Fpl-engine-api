...
const APP_BUILD='2026.08.22.1',API_BASE='https://fpl-engine-api.blackpharoahisking.workers.dev',SCOUT_API_BASE='https://otb-role-intelligence.blackpharoahisking.workers.dev',CACHE_KEY='fpl-engine-cache-v2',STATE_KEY='fpl-engine-user-v2',LEGACY_CACHE_KEY='fpl-engine-rc3-live-cache-v1',LEGACY_STATE_KEY='fpl-engine-rc3-user-v1',EXTERNAL_FIXTURE_KEY='fpl-engine-external-calendar-rc232-v1';
...
/* FPL begins updating cumulative player starts/minutes as soon as a fixture
   starts, while its fixture `finished` flag can remain false until the later
   data-check pass. The role model's denominator must move with those player
   statistics or a live/completed match can produce starts=1 / teamPlayed=0.
   This helper is intentionally separate from final-result logic: autosubs,
   final scores and playerLocked() must continue to require `finished`. */
function fixtureCountsTowardTeamPlayed(f){return Boolean(f?.started||f?.finished)}
function applyFixtures(raw,bootstrap){
  if(!Array.isArray(raw))throw new Error('fixtures response is not an array');
  const teamMap={};
  ...
  for(const f of raw){
    ...
    if(!h||!a)continue;
    (nf[f.event]??=[]).push([h,a]);
    (nm[f.event]??=[]).push({id:f.id,kickoff:f.kickoff_time,hDiff:f.team_h_difficulty,aDiff:f.team_a_difficulty,finished:!!f.finished,started:!!f.started});
    if(fixtureCountsTowardTeamPlayed(f)){
      played[h]=(played[h]||0)+1;
      played[a]=(played[a]||0)+1;
    }
  }
  if(Object.values(nf).flat().length<100)throw new Error('fixture response incomplete');
  FIX=nf;FIX_META=nm;DATA.teamPlayed=played;
  FIX_NOTE=`Official live fixture endpoint loaded: ${Object.values(FIX).flat().length} fixtures across ${Object.keys(FIX).length} gameweeks. Blanks, doubles, postponements, kickoff changes and difficulty ratings will be reflected after refresh.`;
  bumpCache();updateGwSelect();
}
