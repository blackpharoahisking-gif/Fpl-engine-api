...
const APP_RELEASE='RC5.0.27',APP_RELEASE_SLUG='RC5.0.27-review-integrity';
...
function intelligenceCard(row){const evidence=row.evidence||{},sample=evidence.sampleMinutes!=null?`${Math.round(num(evidence.sampleMinutes))} min · ${num(evidence.priorGameweeks)} prior GW`:'completed GW stats',label=String(row.evidenceLabel||'FIRST_WEEK').replaceAll('_',' '),upcoming=intelligenceUpcomingText(row.team);return`<div class="gw-intel-card"><div class="title">${esc(row.name||'—')} · ${esc(row.team||'—')} ${esc(row.position||'')}</div><div class="meta">${num(row.points).toFixed(0)} pts · ${num(row.minutes).toFixed(0)} min · ${num(row.xGI).toFixed(2)} xGI · ${num(row.ownership).toFixed(1)}%</div><div class="why">${esc(row.why||'')}${upcoming?`<br><b>Next:</b> ${esc(upcoming)}`:''}</div><span class="confidence">${esc(label)} EVIDENCE · ${esc(sample)}</span></div>`}
