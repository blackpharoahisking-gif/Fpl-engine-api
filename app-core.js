...
const FPL_SCORING_POLICY_VERSION='fpl-scoring-2026-27-v1',LIMITS={GK:2,DEF:5,MID:5,FWD:3},GOALPTS={GK:10,DEF:6,MID:5,FWD:4},CSPTS={GK:4,DEF:4,MID:1,FWD:0};
...
function chipPrefixForCode(code){
  const normalized=String(code||'').trim().toUpperCase().replace(/[\s-]+/g,'_');
  return({WILDCARD:'WC',WC:'WC',FREE_HIT:'FH',FREEHIT:'FH',FH:'FH',TRIPLE_CAPTAIN:'TC',TRIPLECAPTAIN:'TC','3XC':'TC',TC:'TC',BENCH_BOOST:'BB',BENCHBOOST:'BB',BBOOST:'BB',BB:'BB'})[normalized]||null;
}
function chipKeyFor(code,gw,state=S){
  const target=Math.trunc(num(gw)),prefix=chipPrefixForCode(code);
  if(!prefix||target<1||target>38)return null;
  const key=prefix+(target<=19?'1':'2');
  return key in (state?.chips||{})||key in (state?.usedChips||{})?key:null;
}
function chipUsedGw(key,state=S){const gw=Math.trunc(num(state?.usedChips?.[key]));return gw>=1&&gw<=38?gw:0}
function chipIsUsed(key,state=S){return chipUsedGw(key,state)>0}
function chipStateForGw(gw,state=S){
  const target=Math.trunc(num(gw)),planned=[],used=[];
  const collect=(source,status,out)=>{for(const [key,value] of Object.entries(source||{}))if(Math.trunc(num(value))===target&&target>0){const def=CHIP_STATE_DEFS[key.slice(0,2)];if(def)out.push({key,...def,status,used:status==='USED'})}};
  collect(state?.chips,'PLANNED',planned);collect(state?.usedChips,'USED',used);
  const matches=[...planned,...used],hit=used[used.length-1]||planned[planned.length-1]||null,code=hit?.code||'NONE';
  return{gw:target,key:hit?.key||null,code,label:hit?.label||'No chip',status:hit?.status||'NONE',used:!!hit?.used,active:!!hit,benchScoring:code==='BENCH_BOOST',captainMultiplier:code==='TRIPLE_CAPTAIN'?3:2,conflict:new Set(matches.map(x=>x.key)).size>1,matches};
}
function setChipStateForGw(code,gw,state=S){
  ...
  const key=chipKeyFor(code,target,state);
  if(key&&!chipIsUsed(key,state))state.chips[key]=String(target);
  return chipStateForGw(target,state);
}
function recordUsedChip(code,gw,state=S){
  const target=Math.trunc(num(gw)),key=chipKeyFor(code,target,state);if(!key)return null;
  if(!state.usedChips)state.usedChips={};
  for(const plannedKey of Object.keys(state.chips||{}))if(plannedKey===key||Math.trunc(num(state.chips[plannedKey]))===target)state.chips[plannedKey]='';
  state.usedChips[key]=String(target);
  return{key,gw:target,code:CHIP_STATE_DEFS[key.slice(0,2)]?.code||'NONE'};
}
function replaceUsedChipHistory(rows,teamId=null,state=S){
  const keys=Object.keys(state.chips||{});state.usedChips=Object.fromEntries(keys.map(key=>[key,'']));
  const imported=[];
  for(const row of Array.isArray(rows)?rows:[]){const hit=recordUsedChip(row?.name??row?.chip??row?.code,row?.event??row?.gw,state);if(hit)imported.push(hit)}
  state.chipHistoryTeamId=Number.isFinite(+teamId)&&+teamId>0?Math.trunc(+teamId):null;
  return imported;
}
...
const APP_BUILD=document.documentElement.dataset.build||document.querySelector('meta[name="otb-build"]')?.content||'0.0.0.0',API_BASE='https://fpl-engine-api.blackpharoahisking.workers.dev',SCOUT_API_BASE='https://otb-role-intelligence.blackpharoahisking.workers.dev',CACHE_KEY='fpl-engine-cache-v2',STATE_KEY='fpl-engine-user-v2',FPL_TEAM_ID_KEY='otb-fpl-team-id-v1',LEGACY_CACHE_KEY='fpl-engine-rc3-live-cache-v1',LEGACY_STATE_KEY='fpl-engine-rc3-user-v1',EXTERNAL_FIXTURE_KEY='fpl-engine-external-calendar-rc232-v1';
...
function transferRouteText(plan){if(!Array.isArray(plan)||!plan.length)return'No route';const parts=[];for(const w of plan){if(w.chip){parts.push(`GW${w.gw} ${w.chip}`);continue}if(!w.moves?.length)continue;parts.push(`GW${w.gw} ${w.moves.map(m=>`${m.outName} → ${m.inName}`).join('; ')}`)}return parts.length?parts.join(' · '):`Roll through GW${plan[plan.length-1]?.gw??'—'}`}
...
function renderTransferResult(result,t0,plannerFingerprint=''){
  ...
  const altMarkup=(result.alternatives||[]).map((a,i)=>`<div class="hybrid-alt"><b>Alternative route ${i+1}:</b> ${esc(transferRouteText(a.plan))} · ${num(a.gap).toFixed(2)} hybrid points behind.</div>`).join('');
...
function buildDiscoveryContext(gws){
  const signature=[gws.join(','),S.gw,S.horizon,S.risk,S.squad.join(','),S.transfer.bank==null?'auto':S.transfer.bank,POOL.length,DATA.lastUpdated||DATA.mode].join('|'),cached=globalThis.__DISCOVERY_CONTEXT_CACHE__;
  if(cached?.signature===signature)return cached.ctx;
  const forecast=new Map,fc=p=>{if(!forecast.has(p.id))forecast.set(p.id,discoveryForecast(p,gws));return forecast.get(p.id)};
  // Replacement forecasts are only needed for the points-above-replacement
  // view. Keep its exact cohort/median, but let other views skip that work.
  const replacement={},replacementCohorts={};
  const replacementPlayers=()=>{
    for(const pos of Object.keys(LIMITS)){
      if(replacementCohorts[pos])continue;
      const eligible=POOL.filter(p=>p.p===pos&&TEAMS[p.t]&&availability(p)>=.5),min=eligible.length?Math.min(...eligible.map(p=>p.c)):0,cohort=eligible.filter(p=>p.c<=min+.6);
      replacementCohorts[pos]=cohort.length?cohort:eligible.slice().sort((a,b)=>a.c-b.c).slice(0,Math.max(1,Math.ceil(eligible.length*.15)));
    }
    return Object.values(replacementCohorts).flat();
  };
  for(const pos of Object.keys(LIMITS))Object.defineProperty(replacement,pos,{enumerable:true,configurable:true,get(){
    replacementPlayers();const value=median(replacementCohorts[pos].map(p=>fc(p).total));
    Object.defineProperty(replacement,pos,{enumerable:true,value});return value;
  }});
  const squad=squadPlayers(),clubCounts=squad.reduce((a,p)=>(a[p.t]=(a[p.t]||0)+1,a),{}),transferBank=S.transfer.bank==null?Math.max(0,bank()):Math.max(0,num(S.transfer.bank)),ctx={gws,fc,replacement,replacementPlayers,squad,clubCounts,transferBank,upgrade:new Map};
  globalThis.__DISCOVERY_CONTEXT_CACHE__={signature,ctx};return ctx;
}
...
function renderPool(){
  ...
  const preparation=sort==='replacement'?ctx.replacementPlayers():[];
  if(lowPowerMode()&&(candidates.length+preparation.length)>POOL_RENDER_MOBILE){
    ...
    let index=0,prepIndex=0;
    const scoreSlice=()=>{
      if(computeToken!==POOL_COMPUTE_TOKEN||!columnVisible('colPool'))return;
      const started=performance.now();let count=0;
      while(prepIndex<preparation.length&&count<14&&performance.now()-started<8){ctx.fc(preparation[prepIndex++]);count++}
      while(prepIndex===preparation.length&&index<candidates.length&&count<14&&performance.now()-started<8){scorePlayer(candidates[index++]);count++}
      if(prepIndex<preparation.length||index<candidates.length)requestAnimationFrame(scoreSlice);else finish();
    };
...
function renderChips(){
  try{mountChipSquadPanel()}catch{}
  const defs=[['WC1','Wildcard 1'],['FH1','Free Hit 1'],['TC1','Triple Captain 1'],['BB1','Bench Boost 1'],['WC2','Wildcard 2'],['FH2','Free Hit 2'],['TC2','Triple Captain 2'],['BB2','Bench Boost 2']],keys=defs.map(([key])=>key),host=document.getElementById('chipGrid');
  if(!host)return;
  host.innerHTML=defs.map(([k,label])=>{
    const usedGw=chipUsedGw(k),first=k.endsWith('1'),openingLocked=k.startsWith('FH')||k.startsWith('WC'),lo=first?(openingLocked?2:1):20,hi=first?19:38,id=`chip-${k}`;
    if(usedGw)return`<div class="chipbox"><label class="cn" for="${id}" style="color:var(--muted)">${label}</label><select id="${id}" data-chip="${k}" aria-label="${label} already used in Gameweek ${usedGw}" disabled><option selected>USED · GW${usedGw}</option></select><div class="chip-gw-note" style="font-size:9px;margin-top:4px;color:var(--mint)">Already played · removed from planning</div></div>`;
    let opts='<option value="">—</option>';for(let g=lo;g<=hi;g++)opts+=`<option value="${g}" ${S.chips[k]==g?'selected':''}>GW${g}</option>`;
    return`<div class="chipbox"><label class="cn" for="${id}" style="color:${first?'var(--cyan)':'var(--mint)'}">${label}</label><select id="${id}" data-chip="${k}" aria-label="Plan ${label}">${opts}</select></div>`
  }).join('');
  document.querySelectorAll('[data-chip]').forEach(el=>{const box=el.closest('.chipbox'),k=el.dataset.chip;if(chipIsUsed(k)||!S.chips[k])return;const {doubles,blanks}=gwFixtureShape(+S.chips[k]);if(!doubles&&!blanks)return;const note=document.createElement('div');note.className='chip-gw-note';note.style.cssText='font-size:9px;margin-top:4px;color:'+(doubles?'var(--mint)':'var(--muted)');note.textContent=doubles?`${doubles} club${doubles>1?'s':''} play twice this GW`:blanks?`${blanks} club${blanks>1?'s':''} have no fixture this GW`:'';if(note.textContent)box.appendChild(note)});
  document.querySelectorAll('[data-chip]:not(:disabled)').forEach(el=>el.onchange=e=>{
    const k=e.target.dataset.chip,newVal=e.target.value;
    if(newVal){
      const conflict=keys.find(otherK=>otherK!==k&&String(chipUsedGw(otherK)||S.chips[otherK]||'')===newVal);
      if(conflict){e.target.value=S.chips[k]||'';flash(`GW${newVal} is already assigned to ${conflict}${chipIsUsed(conflict)?' (used)':''} — only one chip per gameweek.`);return}
      const fh1=chipUsedGw('FH1')||num(S.chips.FH1),fh2=chipUsedGw('FH2')||num(S.chips.FH2);
      if(k==='FH1'&&newVal==='19'&&fh2===20){e.target.value=S.chips[k]||'';flash('Free Hit can\'t be played in both GW19 and GW20 — your squad reverts right as GW20 begins.');return}
      if(k==='FH2'&&newVal==='20'&&fh1===19){e.target.value=S.chips[k]||'';flash('Free Hit can\'t be played in both GW19 and GW20 — your squad reverts right as GW20 begins.');return}
    }
    S.chips[k]=newVal;renderChips();saveUserState()
  });
  const firstKeys=['WC1','FH1','TC1','BB1'],remaining=firstKeys.filter(k=>!chipIsUsed(k)),unplanned=remaining.filter(k=>!S.chips[k]),assignments=[...Object.values(S.chips),...Object.values(S.usedChips||{})].filter(Boolean),dupes=assignments.filter((g,i)=>assignments.indexOf(g)!==i),usedFirst=firstKeys.filter(k=>chipIsUsed(k));let v='';
  if(dupes.length)v+=`<div class="verdict warn">Two chip states share <b>GW${dupes[0]}</b>.</div>`;
  if(usedFirst.length)v+=`<div class="verdict"><b>${usedFirst.map(k=>`${chipAdvisorLabel(k)} used GW${chipUsedGw(k)}`).join(' · ')}</b> — ${usedFirst.length===1?'it is':'they are'} no longer available.</div>`;
  v+=unplanned.length?`<div class="verdict warn"><b>${unplanned.length} available first-half chip${unplanned.length>1?'s':''} unplanned.</b></div>`:remaining.length?'<div class="verdict">Every available first-half chip is planned.</div>':'<div class="verdict">All first-half chips have been used.</div>';
  document.getElementById('chipVerdict').innerHTML=v
}
...
function chipAdvisorPayload(){
  ...
  return{players,gws,currentIds:[...S.squad],currentGw:Math.max(1,Number(S.gw)||1),budget:S.budget,fixtureSwings,usedChips:{...S.usedChips}};
}
...
function renderChipAdvisorResult(result){
  ...
  const card=k=>{const r=recs[k]||{},used=!!r.used,guide=!!r.guidance&&!used,none=!r.gw&&!guide&&!used,qualified=r.qualified===true&&!used,badge=used?'Already played · unavailable':guide?'Awaiting BGW/DGW confirmation':none?'No trigger detected':qualified?'Action threshold cleared':'Candidate only · HOLD';return`<div class="chip-ai-card ${(used||!r.gw||!qualified)?'hold':''}"><div class="ca-top"><span class="ca-name">${esc(chipAdvisorLabel(k))}</span><span class="ca-value">${used?'USED':guide?'GUIDANCE':none?'NO TRIGGER':(qualified?'QUALIFIED · ':'CANDIDATE · ')+esc(r.confidence||'—')+(Number.isFinite(+r.score)?' · '+signed(num(r.score),1)+' xP':'')}</span></div><div class="ca-gw">${used?`USED GW${r.gw}`:guide?'TBC':none?'NOT SUGGESTED':`GW${r.gw}`}</div><div class="ca-reason">${esc(r.reason||(k==='WC1'?'Wildcard conditions are not currently met: fewer than 4 owned players have sustained negative swings or fewer than 3 target teams have sustained positive swings.':'No recommendation available.'))}</div><span class="chip-ai-badge ${used?'await':guide?'await':qualified?'strong':'available'}">${esc(badge)}</span></div>`};
  const usedCount=first.filter(k=>recs[k]?.used).length,remaining=first.length-usedCount,actionable=first.filter(k=>recs[k]?.gw&&!recs[k]?.used).length;
  host.innerHTML=`<div class="chip-ai-head"><b>Rule-Based Automated Chip Plan</b><span>${actionable}/${remaining} remaining first-half chips have candidate windows${usedCount?` · ${usedCount} used`:''}</span></div><div class="chip-plan-summary"><b>First-half method:</b> Used chips are removed from consideration. Available chips are assessed from fixture swings, marginal bench value, highest player xP and temporary-squad gain.</div><div class="chip-ai-grid">${first.map(card).join('')}</div><div class="chip-plan-summary"><b>Second half:</b> Specific recommendations are withheld until confirmed blank and double gameweeks are available.</div><div class="chip-ai-grid">${second.map(card).join('')}</div><button type="button" class="btn" id="btnApplyChipAdvice" ${actionable?'':'disabled'}>Apply available first-half recommendations</button><button type="button" class="btn ghost" id="btnRefreshChipAdvice">Recalculate advice</button><div class="chip-ai-note">Imported FPL chip history is authoritative. Applying the plan can update only chips that remain available.</div>`;
  const apply=document.getElementById('btnApplyChipAdvice');if(apply)apply.onclick=()=>{for(const k of first)if(recs[k]?.gw&&!recs[k]?.used&&!chipIsUsed(k))S.chips[k]=String(recs[k].gw);renderChips();saveUserState();renderChipAdvisorResult(result);flash('Available first-half chip recommendations applied. Used chips remained locked.')};
...
function normaliseFplChipHistory(raw){return(Array.isArray(raw?.chips)?raw.chips:Array.isArray(raw)?raw:[]).map(row=>({name:String(row?.name??row?.chip??''),event:fplEventNumber(row?.event??row?.gw)})).filter(row=>row.name&&row.event)}
...
function fplHistoryUrls(teamId){return[`${API_BASE}/api/entry-history?id=${teamId}`,`${API_BASE}/api/entry/${teamId}/history`,`https://fantasy.premierleague.com/api/entry/${teamId}/history/`]}
async function fetchFplChipHistory(teamId,ms=15000){const raw=await fetchFplImportEndpoint(fplHistoryUrls(teamId),ms);if(!Array.isArray(raw?.chips))throw new Error('FPL chip history response did not contain a chips list.');return raw}
async function syncFplUsedChips(teamId,{renderUi=true,persist=true}={}){
  const id=Math.trunc(num(teamId));if(id<1)throw new Error('A valid FPL team id is required to sync chip history.');
  const raw=await fetchFplChipHistory(id),rows=normaliseFplChipHistory(raw),imported=replaceUsedChipHistory(rows,id);
  try{localStorage.setItem(FPL_TEAM_ID_KEY,String(id))}catch{}
  CHIP_ADVISOR_LAST=null;bumpCache();
  if(persist)saveUserState();
  if(renderUi){render();renderChips();scheduleChipAdvisor(0)}
  return{teamId:id,used:imported,rows};
}
...
function applyImportedFplTeam(entry,payload,teamId,importedGw=0,chipHistory=null){const data=normaliseFplPicksPayload(payload),ordered=[...data.picks].sort((a,b)=>num(a.position,99)-num(b.position,99)),mapped=ordered.map(x=>({pick:x,player:fplImportPlayerByElement(x.element)})),missing=mapped.filter(x=>!x.player);if(ordered.length!==15)throw new Error(`The validated public picks response changed and returned ${ordered.length} players instead of 15.`);if(missing.length)throw new Error(`${missing.length} imported player${missing.length===1?' was':'s were'} not recognised in the current OTB player catalogue.`);const players=mapped.map(x=>x.player),ids=players.map(x=>x.id);if(!legal(players))throw new Error('The imported squad failed OTB legal-squad validation.');S.squad=ids;S.start=new Set(mapped.filter(x=>num(x.pick.position,99)<=11).map(x=>x.player.id));if(S.start.size!==11||xiLegality([...S.start])!==null)autoXI();S.cap=mapped.find(x=>x.pick.is_captain)?.player.id??null;S.vice=mapped.find(x=>x.pick.is_vice_captain)?.player.id??null;S.capManual=S.cap!=null;S.viceManual=S.vice!=null;S.benchOrder=mapped.filter(x=>num(x.pick.position)>11).sort((a,b)=>num(a.pick.position)-num(b.pick.position)).map(x=>stableKey(x.player));S.locks=new Set;S.transfer.purchase={};for(const {pick,player} of mapped){const paid=num(pick.purchase_price,NaN);if(Number.isFinite(paid)&&paid>0)S.transfer.purchase[stableKey(player)]=paid/10}const bankRaw=num(data.history.bank,NaN),bank=Number.isFinite(bankRaw)?Math.max(0,bankRaw/10):Math.max(0,bank());S.transfer.bank=bank;const currentValue=players.reduce((a,p)=>a+num(p.c),0);S.budget=Math.round((currentValue+bank)*10)/10;const importedEvent=fplEventNumber(data.history.event)||fplEventNumber(importedGw)||fplEventNumber(entry?.current_event)||Math.max(1,fplEventNumber(DATA.nextEvent)-1);if(chipHistory)replaceUsedChipHistory(normaliseFplChipHistory(chipHistory),teamId);const active=String(data.activeChip||'').toLowerCase(),chipCode=({wildcard:'WILDCARD',freehit:'FREE_HIT',bboost:'BENCH_BOOST','3xc':'TRIPLE_CAPTAIN'})[active]||'NONE';if(chipCode!=='NONE')recordUsedChip(chipCode,importedEvent);try{localStorage.setItem(FPL_TEAM_ID_KEY,String(Math.trunc(num(teamId))))}catch{}CHIP_ADVISOR_LAST=null;ensureCaptainValid();bumpCache();initControls();initTransferControls();render();saveUserState();const name=entry?.name||entry?.player_first_name||`Team ${teamId}`,gw=importedEvent,paidCount=Object.keys(S.transfer.purchase).length,usedChipCount=Object.values(S.usedChips||{}).filter(Boolean).length;return{name,gw,bank,paidCount,usedChipCount,activeChip:chipStateForGw(gw).code}}
async function importFplTeamById(){const input=document.getElementById('fplTeamId'),btn=document.getElementById('btnImportFplTeam'),teamId=Math.trunc(num(input?.value));if(!Number.isFinite(teamId)||teamId<1){setTeamImportStatus('Enter a valid positive FPL manager/team ID.','bad');input?.focus();return}if(btn)btn.disabled=true;setTeamImportStatus(`Finding the newest public squad and chip history for team ${teamId}…`,'warn');try{await ensureImportPlayerMap();const [entry,chipHistory]=await Promise.all([fetchFplImportEndpoint([`${API_BASE}/api/entry?id=${teamId}`,`${API_BASE}/api/entry/${teamId}`,`https://fantasy.premierleague.com/api/entry/${teamId}/`]),fetchFplChipHistory(teamId)]),found=await fetchLatestPublicFplPicks(entry,teamId),result=applyImportedFplTeam(entry,found.payload,teamId,found.gw,chipHistory);setTeamImportStatus(`${result.name} imported from GW${result.gw}: legal 15-player squad, XI, captain, vice-captain, £${result.bank.toFixed(1)}m bank and complete chip history loaded. ${result.usedChipCount?`${result.usedChipCount} used chip${result.usedChipCount===1?' was':'s were'} locked.`:'No chips have been used.'} ${result.paidCount===15?'All purchase prices were available.':`${result.paidCount}/15 purchase prices were available; missing values default to current price.`} Confirm free transfers before planning.`,'good');flash('FPL team and chip history imported successfully.')}catch(e){setTeamImportStatus(`Import failed: ${e.message}`,'bad')}finally{if(btn)btn.disabled=false}}
...
function applyScoutReport(report,{deferCommit=false}={}){if(!report||report.status!=='ok'||!Array.isArray(report.events))throw new Error(report?.error||'Scout response was not recognised.');const team=String(report.team||document.getElementById('riTeam')?.value||'').toUpperCase();const mapped=report.events.map(z=>scoutEventLocal(z,report)).filter(Boolean);const priorWorker=roleIntelEvents().filter(e=>e.worker&&e.team===team);const retained=!scoutReportIsAuthoritative(report)&&!mapped.length&&priorWorker.length>0;const applied=retained?priorWorker:mapped;const manual=roleIntelEvents().filter(e=>!(e.worker&&e.team===team));S.roleIntel.events=[...manual,...applied];SCOUT.last=report;SCOUT.retained=retained;SCOUT.error='';if(!deferCommit)commitScoutReportUpdate();return applied}
function commitScoutReportUpdate(){
  bumpCache();saveUserState();
  // render() already updates the active panel. The hidden Roles panel runs
  // costly counterfactuals that also invalidate caches for every event.
  render();
}
function applyScoutReports(reports){
  let changed=false;
  try{return reports.map(report=>{const events=applyScoutReport(report,{deferCommit:true});changed=true;return events})}
  finally{if(changed)commitScoutReportUpdate()}
}
...
try{const now=Date.now(),rows=Array.from({length:300},(_,i)=>[i+1,2,0,5,80,70,.8,.9,1,null,1,2.125,1.875]),snap={gw:1,release:MODEL_RELEASE,capturedAt:now,deadline:now+864e5,modelFingerprint:'abc12345',rows};snap.checksum=accuracySnapshotChecksum(snap);const clean=sanitizeAccuracyLedger({season:EXPECTED_SEASON,snapshots:{1:snap}}).snapshots[1];add('Projection snapshot checksum survives ledger round-trip',clean.integrity==='verified'&&clean.accountable===true&&clean.rows[0]?.length===13,`${clean.integrity} · ${clean.rows.length} rows · ${clean.rows[0]?.length||0} fields`,'engineering')}catch(e){add('Projection snapshot checksum survives ledger round-trip',false,e.message,'engineering')}
...
function saveCurrentSquad(){
  ...
  const entry={name,ts:Date.now(),spend:+spent().toFixed(1),count:S.squad.length,
    gw:S.gw,horizon:S.horizon,builderStyle:S.builderStyle,snap,chips:{...S.chips},usedChips:{...S.usedChips},chipHistoryTeamId:S.chipHistoryTeamId};
...
function loadSquadByIndex(i){
  ...
  if(e.usedChips){S.usedChips=Object.fromEntries(Object.keys(S.chips).map(key=>[key,e.usedChips[key]||'']));for(const [key,gw] of Object.entries(S.usedChips))if(chipUsedGw(key))recordUsedChip(key.slice(0,2),gw)}
  if(Number.isFinite(+e.chipHistoryTeamId)&&+e.chipHistoryTeamId>0)S.chipHistoryTeamId=Math.trunc(+e.chipHistoryTeamId);
...
const APP_RELEASE='RC5.0.29',APP_RELEASE_SLUG='RC5.0.29-chip-availability';
...
function sanitizeAccuracySnapshotRow(row){const array=Array.isArray(row),get=(i,k)=>array?row[i]:row?.[k],id=Math.trunc(accuracyNumber(get(0,'i'),1,1e9,NaN)),x=accuracyNumber(get(1,'x'),-30,150,NaN);if(!Number.isFinite(id)||!Number.isFinite(x))return null;let low=accuracyNumber(get(2,'l'),-150,150,x),high=accuracyNumber(get(3,'h'),-100,200,x);if(low>x)low=x;if(high<x)high=x;const clean=[id,accuracyRound(x),accuracyRound(low),accuracyRound(high),accuracyRound(accuracyNumber(get(4,'c'),0,100,0),1),accuracyRound(accuracyNumber(get(5,'m'),0,270,0),1),accuracyRound(accuracyNumber(get(6,'ps'),0,1,0),4),accuracyRound(accuracyNumber(get(7,'pa'),0,1,0),4),accuracyRound(accuracyNumber(get(8,'av'),0,1,1),4),accuracyRound(accuracyNumber(get(9,'off'),-20,100,null),2),Math.trunc(accuracyNumber(get(10,'fx'),0,3,1))],hasSd=array?row.length>11:Object.prototype.hasOwnProperty.call(row||{},'sd'),hasNoMarket=array?row.length>12:Object.prototype.hasOwnProperty.call(row||{},'xNoMarket');if(hasSd)clean.push(accuracyRound(accuracyNumber(get(11,'sd'),0,150,null),4));if(hasNoMarket){if(!hasSd)clean.push(null);clean.push(accuracyRound(accuracyNumber(get(12,'xNoMarket'),-30,150,null)))}return clean}
...
function accuracyProjectionSettings(){const weights=Object.fromEntries(Object.entries(S.w||{}).sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>[k,accuracyRound(num(v),4)])),overrides={};for(const [k,v] of Object.entries(S.overrides||{}).sort(([a],[b])=>a.localeCompare(b))){const z={};for(const f of ['availability','start','minutes','role'])if(v?.[f]!==undefined&&v[f]!=='')z[f]=accuracyRound(num(v[f]),4);if(Object.keys(z).length)overrides[k]=z}const strengths=Object.keys(TEAMS).sort().map(c=>[c,accuracyRound(TEAMS[c]?.s,3),accuracyRound(TEAMS[c]?.atkH,3),accuracyRound(TEAMS[c]?.atkA,3),accuracyRound(TEAMS[c]?.defH,3),accuracyRound(TEAMS[c]?.defA,3)]),roleEvents=roleIntelEvents().slice().sort((a,b)=>String(a.id).localeCompare(String(b.id))).map(e=>[accuracyText(e.id,80),accuracyNumber(e.createdAt,0,9e15,0),accuracyText(e.team,4),accuracyText(e.type,32),accuracyText(e.affectedKey,120),accuracyNumber(e.overlap,-10,10,0),accuracyNumber(e.hierarchy,-10,10,0),accuracyNumber(e.confidence,-10,10,0),accuracyNumber(e.minuteCeiling,-1,270,null),accuracyNumber(e.startOverride,-1,1,null),accuracyNumber(e.productionImpact,-1,1,null),!!e.worker,accuracyText(e.source,160)]),market={loaded:!!MARKET.loaded,usable:!!MARKET.loaded&&!marketStale(),fetchedAt:num(MARKET.fetchedAt),ageMinutes:accuracyRound(MARKET.ageMinutes,2),fixtures:num(MARKET.fixtures),slateHash:accuracyHashValue(MARKET.slate||[])};return{weights,overrides,strengths,roleEvents,market,scoringPolicy:FPL_SCORING_POLICY_VERSION}}
...
function intelligenceCard(row){const evidence=row.evidence||{},sample=evidence.sampleMinutes!=null?`${Math.round(num(evidence.sampleMinutes))} min · ${num(evidence.priorGameweeks)} prior GW`:'completed GW stats',label=String(row.evidenceLabel||'FIRST_WEEK').replaceAll('_',' '),upcoming=intelligenceUpcomingText(row.team),p=row.persistence||null,pStatus=String(p?.status||'').toUpperCase(),pClass=pStatus.toLowerCase(),pGws=Math.max(0,Math.round(num(p?.sampleGameweeks))),pHits=Math.max(0,Math.round(num(p?.signalGameweeks))),pStarts=Math.max(0,Math.round(num(p?.rollingStarts))),persistence=pStatus&&pGws?`<div class="gw-intel-persistence ${esc(pClass)}"><b>${esc(pStatus)} SIGNAL</b> · ${pHits} of ${pGws} sampled GW${pGws===1?'':'s'}<span>Rolling ${pGws} GW${pGws===1?'':'s'}: ${num(p?.rollingXGI).toFixed(2)} xGI · ${Math.round(num(p?.rollingMinutes))} min · ${pStarts} start${pStarts===1?'':'s'}</span></div>`:'',ownershipLabel=row.signal==='LOW_OWNED_EMERGING'?' owned at review capture':'';return`<div class="gw-intel-card"><div class="title">${esc(row.name||'—')} · ${esc(row.team||'—')} ${esc(row.position||'')}</div><div class="meta">${num(row.points).toFixed(0)} pts · ${num(row.minutes).toFixed(0)} min · ${num(row.xGI).toFixed(2)} xGI · ${num(row.ownership).toFixed(1)}%${ownershipLabel}</div>${persistence}<div class="why">${esc(row.why||'')}${upcoming?`<br><b>Next:</b> ${esc(upcoming)}`:''}</div><span class="confidence">${esc(label)} EVIDENCE · ${esc(sample)}</span></div>`}
...
function intelligenceTeamTrends(report){
  const host=document.getElementById('intelligenceTeams');if(!host)return;
  const rows=report.teamTrends||[];
  if(!rows.length){host.innerHTML='';return}
  const display=(row,side)=>{
    const attack=side==='attack',label=attack?'ATK':'DEF',direction=row[attack?'attackDirection':'defenseDirection'],level=row[attack?'attackLevel':'defenseLevel'];
    if(direction==='BLANK'||level==='BLANK')return`<span class="gw-intel-steady">${label} —</span>`;
    const levelText=attack?({ABOVE:'HIGH',AVERAGE:'AVG',BELOW:'LOW'}[level]||'AVG'):({STRONG:'STRONG',AVERAGE:'AVG',WEAK:'WEAK'}[level]||'AVG');
    const trend=['UP','DOWN','STEADY'].includes(direction)?` ${direction==='UP'?'↑':direction==='DOWN'?'↓':'→'}`:'';
    const tone=['UP'].includes(direction)||(direction==='INSUFFICIENT'&&((attack&&level==='ABOVE')||(!attack&&level==='STRONG'))) ? 'up'
      : ['DOWN'].includes(direction)||(direction==='INSUFFICIENT'&&((attack&&level==='BELOW')||(!attack&&level==='WEAK'))) ? 'down' : 'steady';
    return`<span class="gw-intel-${tone}">${label} ${esc(levelText)}${trend}</span>`;
  };
  const hasTrend=rows.some(row=>['UP','DOWN','STEADY'].includes(row.attackDirection)||['UP','DOWN','STEADY'].includes(row.defenseDirection));
  host.innerHTML=`<div class="sechead" style="margin:12px -14px 8px;position:static"><h2>Team process</h2><span class="note">${hasTrend?'level + multi-GW trend':'single-GW level'} · per-fixture xG / xGC</span></div><div class="accuracy-table"><div class="gw-intel-team" style="color:var(--cyan);font-weight:800"><span>Club</span><span>Attack</span><span>Defence</span><span>xG/xGC</span></div>${rows.map(row=>`<div class="gw-intel-team"><b>${esc(row.team)}</b>${display(row,'attack')}${display(row,'defense')}<span class="mono">${row.fixtures?`${num(row.xG).toFixed(2)}/${num(row.xGC).toFixed(2)}`:'BLANK'}</span></div>`).join('')}</div><div class="accuracy-note">HIGH / AVG / LOW and STRONG / AVG / WEAK describe the selected Gameweek level. Arrows appear only after at least two earlier completed Gameweeks and then describe change versus that prior sample.</div>`
}
...
async function syncSquadRoleIntelligence(){
  ...
  const accepted=Object.entries(reports).filter(([code,report])=>report?.status==='ok'&&Array.isArray(report.events)).map(([code,report])=>({...report,team:report.team||code}));
  if(!accepted.length)return {status:'empty',applied:0};
  const applied=applyScoutReports(accepted);
  SCOUT.syncedAt=Date.now();SCOUT.syncedTeams=accepted.map(report=>report.team);
  return {status:'ok',applied:applied.flat().length,teams:accepted.length};
}
