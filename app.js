...
function fixtureListFor(team,gw){const out=[];(FIX[gw]||[]).forEach(([h,a],i)=>{const m=(FIX_META[gw]||[])[i]||{};if(h===team)out.push({opp:a,home:true,difficulty:m.hDiff,kickoff:m.kickoff,finished:m.finished,id:m.id});else if(a===team)out.push({opp:h,home:false,difficulty:m.aDiff,kickoff:m.kickoff,finished:m.finished,id:m.id})});return out}
...
function freshReviewAccessLogin(){window.open(`${SCOUT_API_BASE}/api/fresh-review/session`,'otbFreshReviewAccess','noopener,noreferrer');flash('Complete the Cloudflare Access email sign-in, then return here and press Check sign-in.')}
function freshReviewAccessLogout(){try{sessionStorage.removeItem(FRESH_REVIEW_TOKEN_KEY)}catch(e){}FRESH_SQUAD_REVIEW.identity=null;window.open(`${SCOUT_API_BASE}/cdn-cgi/access/logout`,'otbFreshReviewLogout','noopener,noreferrer');renderFreshSquadReview(verdictContext())}
async function freshReviewApi(path,{method='GET',body=null,timeout=50000}={}){
  ...
  try{
    ...
    const data=await response.json().catch(()=>({}));if(response.status===401){if(token)try{sessionStorage.removeItem(FRESH_REVIEW_TOKEN_KEY)}catch(e){}const error=new Error('Cloudflare Access sign-in is required.');error.code='AUTH';throw error}if(!response.ok)throw new Error(data.error||`Fresh Review HTTP ${response.status}`);return data;
  }catch(e){if(e?.name==='AbortError')throw new Error('The status request timed out. The Cloudflare job is still safe to close and can be resumed later.');if(e?.code==='AUTH')throw e;if(e instanceof TypeError)throw new Error('Fresh Review could not reach the signed-in Worker. Sign in with Cloudflare Access, then retry.');throw e}finally{clearTimeout(timer)}
}
async function freshReviewCheckSession(){try{const session=await freshReviewApi('/api/fresh-review/session',{timeout:20000});FRESH_SQUAD_REVIEW.identity=session.identity||null;if(session.identity?.mode==='cloudflare-access')try{sessionStorage.removeItem(FRESH_REVIEW_TOKEN_KEY)}catch(e){}FRESH_SQUAD_REVIEW.error='';renderFreshSquadReview(verdictContext());flash(session.identity?.email?`Fresh Review signed in as ${session.identity.email}.`:'Fresh Review access is active.');return true}catch(e){FRESH_SQUAD_REVIEW.error=e?.message||String(e);renderFreshSquadReview(verdictContext());return false}}
...
function applyFixtures(raw,bootstrap){if(!Array.isArray(raw))throw new Error('fixtures response is not an array');const teamMap={};(bootstrap?.teams||[]).forEach(t=>teamMap[t.id]=t.short_name);const nf={},nm={},played={};for(const f of raw){if(!f.event)continue;const h=teamMap[f.team_h],a=teamMap[f.team_a];if(!h||!a)continue;(nf[f.event]??=[]).push([h,a]);(nm[f.event]??=[]).push({id:f.id,kickoff:f.kickoff_time,hDiff:f.team_h_difficulty,aDiff:f.team_a_difficulty,finished:!!f.finished,started:!!f.started});if(f.finished){played[h]=(played[h]||0)+1;played[a]=(played[a]||0)+1}}if(Object.values(nf).flat().length<100)throw new Error('fixture response incomplete');FIX=nf;FIX_META=nm;DATA.teamPlayed=played;FIX_NOTE=`Official live fixture endpoint loaded: ${Object.values(FIX).flat().length} fixtures across ${Object.keys(FIX).length} gameweeks. Blanks, doubles, postponements, kickoff changes and difficulty ratings will be reflected after refresh.`;bumpCache();updateGwSelect()}
...
try{add('Final-result gate waits for FPL data checks',accuracyEventFinal({finished:true,data_checked:true})&&!accuracyEventFinal({finished:true,data_checked:false})&&!accuracyEventFinal({finished:false,data_checked:true}),'finished + data checked required','engineering')}catch(e){add('Final-result gate waits for FPL data checks',false,e.message,'engineering')}
...
try{const api=String(freshReviewApi),runner=String(runFreshSquadReview),page=document.documentElement.outerHTML;
  add('Live review uses Cloudflare Access transport without embedded credentials',api.includes('SCOUT_API_BASE+path')&&api.includes("credentials:'include'")&&runner.includes('/api/fresh-review')&&!/sk-[A-Za-z0-9_-]{16,}|OPENAI_API_KEY\s*=|FRESH_REVIEW_OWNER_SECRET\s*=/.test(page),'Access cookie transport · protected Worker route · no secret literal','engineering')}
...
try{const runner=String(runFreshSquadReview),poller=String(pollFreshSquadReview),owner=String(freshReviewOwnerToken);
  ...
  add('Fresh Review no longer prompts for a long owner capability',!owner.includes('window.prompt')&&String(freshReviewAccessLogin).includes('/api/fresh-review/session'),'Cloudflare Access sign-in · no manual capability prompt','engineering')}
catch(e){add('Fresh Review durable mobile execution and Access login',false,e.message,'engineering')}
...
document.getElementById('accuracyGw').addEventListener('change',renderAccuracy);
document.getElementById('accuracyCohort').addEventListener('change',renderAccuracy);
document.getElementById('btnAccuracySnapshot').onclick=()=>{try{captureProjectionSnapshot(num(document.getElementById('accuracyGw').value),{manual:true});renderAccuracy()}catch(e){ACCURACY.error=e.message;renderAccuracy()}};
...
const APP_RELEASE='RC5.0.24',APP_RELEASE_SLUG='RC5.0.24-durable-fresh-review';
const ACCURACY_LEDGER_KEY='fpl-engine-projection-ledger-rc233-v1',ACCURACY_SCHEMA=2,MODEL_RELEASE=APP_RELEASE;  /* stamped into every accuracy snapshot - must track the real model build */

function emptyAccuracyLedger(){return{schema:ACCURACY_SCHEMA,season:EXPECTED_SEASON,snapshots:{},actuals:{},catalog:{}}}
const ACCURACY={ledger:emptyAccuracyLedger(),loading:false,error:''};
const ACCURACY_SYNC_ATTEMPTS=new Map(),ACCURACY_AUTO_RETRY_MS=15*60*1000;let ACCURACY_CAPTURE_TIMER=null;function scheduleAccuracyCapture(delay=650){clearTimeout(ACCURACY_CAPTURE_TIMER);ACCURACY_CAPTURE_TIMER=null;if(lowPowerMode())return;ACCURACY_CAPTURE_TIMER=setTimeout(()=>{ACCURACY_CAPTURE_TIMER=null;maybeAutoCaptureProjection()},Math.max(0,delay))}
...
function accuracyEventFinal(e){return!!(e?.finished&&e?.data_checked!==false)}function accuracyFinished(gw){return accuracyEventFinal(accuracyEvent(gw))}
...
function accuracyProjectionSettings(){const weights=Object.fromEntries(Object.entries(S.w||{}).sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>[k,accuracyRound(num(v),4)])),overrides={};for(const [k,v] of Object.entries(S.overrides||{}).sort(([a],[b])=>a.localeCompare(b))){const z={};for(const f of ['availability','start','minutes','role'])if(v?.[f]!==undefined&&v[f]!=='')z[f]=accuracyRound(num(v[f]),4);if(Object.keys(z).length)overrides[k]=z}const strengths=Object.keys(TEAMS).sort().map(c=>[c,accuracyRound(TEAMS[c]?.s,3),accuracyRound(TEAMS[c]?.atkH,3),accuracyRound(TEAMS[c]?.atkA,3),accuracyRound(TEAMS[c]?.defH,3),accuracyRound(TEAMS[c]?.defA,3)]);return{weights,overrides,strengths}}
...
function projectionSnapshotRows(gw){
  const base=POOL.filter(p=>p.apiId!=null&&TEAMS[p.t]);
  // Counterfactual pass first: suspend the blend, flush caches, project.
  const noMarket=new Map();
  const wasSuspended=MARKET_SUSPEND;
  try{
    MARKET_SUSPEND=true;bumpCache();
    for(const p of base){const r=project(p,gw);noMarket.set(p.apiId,r&&r.x)}
  }catch(err){
    console.warn('OTB no-market counterfactual skipped:',err);
    noMarket.clear();
  }finally{
    MARKET_SUSPEND=wasSuspended;bumpCache();
  }
  return base.map(p=>{
    const r=project(p,gw),md=minuteDetail(p),usage=accuracyEventUsage(md,r.fixtures?.length||0);
    const cf=noMarket.has(p.apiId)?noMarket.get(p.apiId):null;
    return [Math.trunc(num(p.apiId)),accuracyRound(r.x),accuracyRound(r.low),accuracyRound(r.high),
      accuracyRound(r.confidence,1),accuracyRound(usage.minutes,1),accuracyRound(usage.pStart,4),
      accuracyRound(usage.pAppear,4),accuracyRound(md.avail,4),accuracyRound(p.live?.epNext,2),
      usage.fixtures,accuracyRound(r.sd,4),Number.isFinite(cf)?accuracyRound(cf):null];
  }).filter(row=>Number.isFinite(row[0])&&row[0]>0&&Number.isFinite(row[1]));
}
function captureProjectionSnapshot(gw,{manual=false}={}){gw=num(gw,DATA.nextEvent||S.gw);const deadline=accuracyDeadline(gw),now=Date.now();if(!Number.isFinite(deadline))throw new Error(`GW${gw} deadline is not verified.`);if(now>=deadline)throw new Error(`GW${gw} deadline has passed. Historical forecasts cannot be recreated after the fact.`);if(!['LIVE','CACHE'].includes(DATA.mode))throw new Error('Live or validated cached FPL data are required for an accountable snapshot.');if(!DATA.validation?.topologyPass)throw new Error('Fixture topology must pass before a snapshot is recorded.');const rows=projectionSnapshotRows(gw);if(rows.length<300)throw new Error(`Only ${rows.length} players were available; snapshot rejected as incomplete.`);const catalog={};for(const p of POOL)if(p.apiId!=null){const row={n:p.n,t:p.t,p:p.p};catalog[p.apiId]=row;ACCURACY.ledger.catalog[p.apiId]=row}const settings=accuracyProjectionSettings(),modelFingerprint=accuracyHashValue(settings),old=ACCURACY.ledger.snapshots[gw],snapshot={gw,release:MODEL_RELEASE,capturedAt:now,firstCapturedAt:old?.firstCapturedAt||now,updates:num(old?.updates)+1,deadline,dataMode:DATA.mode,dataUpdatedAt:DATA.lastUpdated||null,historicalCoverage:accuracyRound(DATA.histCoverage?.eligibleRatio,4),historicalCoverageOverall:accuracyRound(DATA.histCoverage?.overallRatio,4),historyMatched:num(DATA.histCoverage?.matched),historyEligible:num(DATA.histCoverage?.eligible),historyPriorFree:num(DATA.histCoverage?.newcomer),weights:{...S.w},overrideCount:Object.keys(settings.overrides).length,rows,catalog,settings,modelFingerprint,origin:'local-capture',integrity:'verified',accountable:true};snapshot.checksum=accuracySnapshotChecksum(snapshot);ACCURACY.ledger.snapshots[gw]=snapshot;if(!saveAccuracyLedger()){if(old)ACCURACY.ledger.snapshots[gw]=old;else delete ACCURACY.ledger.snapshots[gw];throw new Error(`Snapshot created but could not be saved: ${ACCURACY.error}`)}if(manual)flash(`GW${gw} projection snapshot captured for ${rows.length} players.`);return snapshot}
function maybeAutoCaptureProjection(){try{if(lowPowerMode())return false;const gw=num(DATA.nextEvent);if(!gw||!['LIVE','CACHE'].includes(DATA.mode)||!DATA.validation?.topologyPass)return false;const deadline=accuracyDeadline(gw);if(!Number.isFinite(deadline)||Date.now()>=deadline)return false;const old=ACCURACY.ledger.snapshots[gw],fingerprint=accuracyHashValue(accuracyProjectionSettings()),stale=!old||old.accountable===false||Date.now()-num(old.capturedAt)>6*36e5||old.release!==MODEL_RELEASE||old.dataUpdatedAt!==DATA.lastUpdated||old.modelFingerprint!==fingerprint;if(stale){captureProjectionSnapshot(gw);if(activeRailTab()==='accuracy')renderAccuracy();return true}}catch(e){ACCURACY.error=e.message}return false}
...
function storeAccuracyActuals(gw,rows,source='manual'){gw=num(gw);const packed=accuracyDedupeRows(rows.map(r=>Array.isArray(r)?r:[r.i,r.pts,r.min,r.app===null?null:(r.app?1:0),r.start===null?null:(r.start?1:0)]),sanitizeAccuracyActualRow);if(packed.length<300)throw new Error(`Only ${packed.length} unique result rows recognised; a complete official Gameweek payload is required.`);const old=ACCURACY.ledger.actuals[gw],record={gw,source:accuracyText(source,160),importedAt:Date.now(),final:accuracyFinished(gw),finished:accuracyFinished(gw),rows:packed,complete:true,integrity:'verified'};record.checksum=accuracyActualChecksum(record);ACCURACY.ledger.actuals[gw]=record;ACCURACY.error='';if(!saveAccuracyLedger()){if(old)ACCURACY.ledger.actuals[gw]=old;else delete ACCURACY.ledger.actuals[gw];throw new Error(`Results parsed but could not be saved: ${ACCURACY.error}`)}return record}
...
async function fetchAccuracyActualsForGw(gw){const urls=[`${API_BASE}/api/event-live?gw=${gw}`,`${API_BASE}/event/${gw}/live/`,`https://fantasy.premierleague.com/api/event/${gw}/live/`];let last=null;for(const url of urls){try{const data=await fetchJSON(url,15000),rows=actualRowsFromPayload(data);if(rows.length>=300){storeAccuracyActuals(gw,rows,url.startsWith(API_BASE)?'worker':'official FPL');return rows}}catch(e){last=e}}throw last||new Error('No supported results endpoint returned a complete payload')}
async function syncAccuracyActuals(){const gw=num(document.getElementById('accuracyGw')?.value,S.gw),btn=document.getElementById('btnAccuracySync');if(btn)btn.disabled=true;setAccuracyStatus(`Checking official GW${gw} results…`,'warn');try{const rows=await fetchAccuracyActualsForGw(gw);renderAccuracy();flash(`GW${gw} results loaded for ${rows.length} players.`)}catch(e){ACCURACY.error=e.message;setAccuracyStatus(`Automatic result sync failed: ${e.message}. Paste official event-live JSON in the manual import section.`,'bad')}finally{if(btn)btn.disabled=false}}
...
function renderAccuracy(){ensureAccuracyGwSelect();const cohort=document.getElementById('accuracyCohort')?.value||'relevant',gw=num(document.getElementById('accuracyGw')?.value,DATA.nextEvent||S.gw),snap=ACCURACY.ledger.snapshots[gw],act=ACCURACY.ledger.actuals[gw],deadline=accuracyDeadline(gw),canCapture=Number.isFinite(deadline)&&Date.now()<deadline&&['LIVE','CACHE'].includes(DATA.mode)&&!!DATA.validation?.topologyPass,cap=document.getElementById('btnAccuracySnapshot');if(cap){cap.disabled=!canCapture;cap.title=canCapture?'Capture or update this accountable forecast before the deadline':'Requires a future verified deadline, valid live/cache data and passed fixture topology'}if(ACCURACY.error)setAccuracyStatus(ACCURACY.error,'bad');else if(accuracySnapshotReady(gw)&&accuracyActualReady(gw))setAccuracyStatus(`GW${gw} is fully evaluated. Forecast captured ${new Date(snap.capturedAt).toLocaleString()} using ${snap.release}; ${act.rows.length} final result rows loaded.`,'good');else if(snap&&!accuracySnapshotReady(gw))setAccuracyStatus(`GW${gw} contains an imported or damaged forecast that failed accountability checks (${snap.integrity||'incomplete'}). It is excluded from performance totals.`,'bad');else if(snap&&act)setAccuracyStatus(accuracyFinished(gw)?`GW${gw} has provisional or incomplete results and needs a final refresh before evaluation.`:`GW${gw} results are provisional and excluded from performance totals until the event is finished and refreshed.`,'warn');else if(snap)setAccuracyStatus(`GW${gw} forecast is stored for ${snap.rows.length} players and becomes immutable after ${new Date(snap.deadline).toLocaleString()}. Final official results are still required.`,'warn');else if(Number.isFinite(deadline)&&Date.now()<deadline)setAccuracyStatus(lowPowerMode()?`GW${gw} has no stored snapshot yet. Mobile fast mode leaves this expensive full-player pass manual; tap Capture when you want to record it.`:`GW${gw} has no stored snapshot yet. Valid data will maintain the next Gameweek automatically; this button can record an earlier forecast for another future Gameweek.`,'warn');else setAccuracyStatus(`GW${gw} has no accountable pre-deadline forecast. OTB will not manufacture one after the deadline.`,'bad');renderAccuracySummary(cohort);renderAccuracyGwTable(cohort);renderAccuracyPositions(cohort)}
