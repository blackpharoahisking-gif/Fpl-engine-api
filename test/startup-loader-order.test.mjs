...
test('production boot appends live/core path first and release helper last',()=>{
  ...
  assert.equal(html.dataset.build,'2026.08.29.1');
  assert.equal(meta.content,'2026.08.29.1');
  assert.equal(badge.textContent,'BUILD 08.29.1');
  assert.equal(appended.length,1,'nothing may run ahead of the live/core loader');
  assert.equal(appended[0].src,'app-live-points.js?v=2026.08.29.1-live');
  ...
  assert.deepEqual(
    ...
    [
      ...
      'release-identity.js?v=2026.08.29.1-release',
