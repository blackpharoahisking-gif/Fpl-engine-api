...
test('production startup preserves live/core first and decision integrity downstream of governance',()=>{
  assert.match(loader,/const BUILD='2026\.08\.27\.2'/);
  const liveAt=loader.indexOf("app-live-points.js?v=2026.08.27.2-live");
  ...
  const releaseAt=loader.indexOf("release-identity.js?v=2026.08.27.2-release");
  ...
  assert.match(live,/script\.src='app-core\.js\?v=2026\.08\.27\.2-core'/,'live layer must still own the established core append path');
});
...
test('an older runtime identity and bookmarked build query are upgraded to 08.27.2',()=>{
  const r=runHelper();
...
test('the helper never downgrades a future build',()=>{
  const r=runHelper({
    htmlBuild:'2026.08.27.3',
    metaBuild:'2026.08.27.3',
    badge:'BUILD 08.27.3',
    url:'https://example.test/FPL_Engine_OTB.html?build=2026.08.27.3&reload=123',
  });
  assert.equal(r.html.dataset.build,'2026.08.27.3');
  assert.equal(r.meta.content,'2026.08.27.3');
  assert.equal(r.badgeNode.textContent,'BUILD 08.27.3');
  assert.equal(new URL(r.href).searchParams.get('build'),'2026.08.27.3');
  assert.equal(r.context.__OTB_RELEASE_IDENTITY__.current,'2026.08.27.3');
});
