...
test('production startup preserves the live/core-first order with role freshness and accountability downstream',()=>{
  assert.match(loader,/const BUILD='2026\.08\.26\.9'/);
  const liveAt=loader.indexOf("app-live-points.js?v=2026.08.26.9-live");
  ...
  const releaseAt=loader.indexOf("release-identity.js?v=2026.08.26.9-release");
  ...
  assert.ok(releaseAt>v2At,'release metadata helper must be last and non-critical');
...
test('an older runtime identity and bookmarked build query are upgraded to .9',()=>{
  const r=runHelper();
  assert.equal(r.html.dataset.build,'2026.08.26.9');
  assert.equal(r.meta.content,'2026.08.26.9');
  assert.equal(r.badgeNode.textContent,'BUILD 08.26.9');
  assert.equal(new URL(r.href).searchParams.get('build'),'2026.08.26.9');
  assert.equal(r.context.__OTB_RELEASE_IDENTITY__.current,'2026.08.26.9');
});

test('the helper never downgrades a future build',()=>{
  const r=runHelper({
    htmlBuild:'2026.08.26.10',
    metaBuild:'2026.08.26.10',
    badge:'BUILD 08.26.10',
    url:'https://example.test/FPL_Engine_OTB.html?build=2026.08.26.10&reload=123',
  });
  assert.equal(r.html.dataset.build,'2026.08.26.10');
  assert.equal(r.meta.content,'2026.08.26.10');
  assert.equal(r.badgeNode.textContent,'BUILD 08.26.10');
  assert.equal(new URL(r.href).searchParams.get('build'),'2026.08.26.10');
  assert.equal(r.context.__OTB_RELEASE_IDENTITY__.current,'2026.08.26.10');
});
