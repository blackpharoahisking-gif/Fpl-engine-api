...
test('production startup preserves the live/core-first order with accountability downstream',()=>{
  assert.match(loader,/const BUILD='2026\.08\.26\.8'/);
  const liveAt=loader.indexOf("app-live-points.js?v=2026.08.26.8-live");
  ...
  const releaseAt=loader.indexOf("release-identity.js?v=2026.08.26.8-release");
  ...
  assert.ok(v2At>marketAt,'accountability v2 must remain downstream of core/model layers');
...
test('an older runtime identity and bookmarked build query are upgraded to .8',()=>{
  const r=runHelper();
...
test('the helper never downgrades a future build',()=>{
  const r=runHelper({
    htmlBuild:'2026.08.26.9',
    metaBuild:'2026.08.26.9',
    badge:'BUILD 08.26.9',
    url:'https://example.test/FPL_Engine_OTB.html?build=2026.08.26.9&reload=123',
  });
